describe('Episodes Page', () => {
  ...
  it('should show no episodes message when empty', () => {
    ...
    cy.intercept('GET', '**/v1/episodes/', { fixture: 'empty-episodes.json' }).as('getEpisodes')
    
    cy.visit('/')
    cy.wait('@getEpisodes')
    
    cy.contains('No episodes found. Create your first episode!').should('be.visible')
  })

  it('should display episodes when data is available', () => {
    // Mock episodes response with sample data
    cy.intercept('GET', '**/v1/episodes/', { fixture: 'episodes.json' }).as('getEpisodes')
